...
const targetImport = `export function BookingFlow() {`;
...
code = code.replace(targetImport, beforeComponent);

// Also remove the `loadSlots()` call from the catch block
code = code.replace('loadSlots();', 'loadBarberData();');
